SELECT 
    ...
    PRD.REFRIG_CD,
    PRD.TRACKABLE_ITEM,
    MAX_PRD.PROD_GRP_ID AS PROD_GRP_PRNT_ID,
    ALT_PRD.CNTRY_CD,
    MAX_PRD.GRP_PRPS_CD,
    MAX_PRD.END_DT AS END_DT_PROD_GRP_PROD,
    MAX_PRD.CEASE_PBLSH_DT AS CEASE_PBLSH_DT_PROD_GRP_PROD
FROM (
    SELECT BRND_CD, COLLAB_IND, CRT_DT, DMND_DFLT_CNVRSN_FCTR, DMND_DFLT_CNVRSN_UOM_CD, 
...
JOIN (
    SELECT DISTINCT A.PROD_ID, B.PROD_GRP_ID, B.GRP_PRPS_CD, C.END_DT,
...
JOIN (
    SELECT DISTINCT A.PROD_ID, B.CNTRY_CD, MAX(B.LST_PBLSH_DT) AS LST_PBLSH_DT
    FROM gs_edb_cms.MTM_GH_PROD_VW A
...
WHERE PRD.PROD_TYP IN ('SAMPLE', 'EDUCATIONAL')
