"""Glue Script to load GSO_PROD_RPLCTN table in RDS from Json file in S3
...
logger.info("Starting gso_ib_prod_rpl Glue job")
...
def init_config(config_common):
    ...
    Args:
        config_common : common configuration details
    """
    try:
...
def ingest_data_trunc_load(df, table_name):
    ...
    Args:
        ...
        table_name : Table name to be ingested
    """
    options = {
...
def transform(df):
    ...
    df = df.select(
        ...
        col("Prod_id__c").alias("PROD_ID_GLBL__C"),
        col("Product_External_ID__c").alias("PRODUCT_EXTERNAL_ID__C"),
        col("Cntry_Cd__c").alias("CNTRY_CD_GLBL__C"))
...
def move_from_raw_to_archieve(bucket_name,directory_path,archive_path):
    ...
    Args:
        bucket_name : S3 Bucket Name
        directory_path : Directory Folder Path
...
def main():
    ...
    try:
        ...
        logger.info("gso_ib_prod_rpl Glue job completed")
        spark.stop()
